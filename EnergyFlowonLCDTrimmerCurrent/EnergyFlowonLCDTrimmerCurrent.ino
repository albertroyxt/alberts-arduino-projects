...
float CtestFrequency = 60;                     // test signal frequency (Hz)
float CwindowLength = 20.0/CtestFrequency;     // how long to average the signal, for statistist
int CsensorValue = 0;
float Cintercept = -0.01; // to be adjusted based on calibration testing
float Cslope = 0.0405; // to be adjusted based on calibration testing
float Ccurrent_amps; // estimated actual current in amps

unsigned long CprintPeriod = 1000; // in milliseconds
...
RunningStatistics inputStats;                 // create statistics to look at the raw test signal
  inputStats.setWindowSecs( CwindowLength );
   
  while( true ) {   
    CsensorValue = analogRead(A15);  // read the analog in value:
    inputStats.input(CsensorValue);  // log to Stats function
        
    if((unsigned long)(millis() - CpreviousMillis) >= CprintPeriod) {
      CpreviousMillis = millis();   // update time
      ...
      Ccurrent_amps = Cintercept + Cslope * inputStats.sigma();
      Serial.print( "\tamps: " ); Serial.print( Ccurrent_amps );
    }
  } */
