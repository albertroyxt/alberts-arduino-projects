...
void loop()
{
 ...
 struct roverRemoteData payload;// In this section is where you would load the data that needs to be sent.
  ...
  payload.Sensor1Data =switchVal;// analogRead(Sensor1Pin);
  payload.Sensor2Data =value;// analogRead(Sensor2Pin);
  payload.Sensor3Data =limit;// analogRead(Sensor3Pin);
payload.Sensor4Data =limit;
